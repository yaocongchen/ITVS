...
if button == 1:
    ...
    engine.setProperty('voice', zh_tw_voice_id)
    count =True
...
def speak(sentence):
	with tempfile.NamedTemporaryFile(delete=True) as fp:
        tts = engine.setProperty('voice', Language)
...
#讀取校正檔座標值
f = filter(None, (line.rstrip() for line in open(r'/home/yaocong/topicNOINTERNET/table.txt')))     #讀取檔案，但將多餘的空行去除
lines=0
text =[]                                                #設置文字陣列
...
#讀取球體運動檔座標值
f = filter(None, (line.rstrip() for line in open(r'/home/yaocong/topicNOINTERNET/ball.txt')))     #讀取檔案，但將多餘的空行去除
lines=0
text =[]                                                #設置文字陣列
for line in f:                                          #讀取文字迴圈
    text.append(line)                                   #將文字逐行放入text陣列
